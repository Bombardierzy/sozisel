defmodule Sozisel.ExMachina.PolymorphicEcto do
  defmacro __using__(opts) do
    verify_ecto_dep()

    quote do
      use ExMachina
      use Sozisel.ExMachina.PolymorphicEctoStrategy, repo: unquote(Keyword.get(opts, :repo))

      def params_for(factory_name, attrs \\ %{}) do
        Sozisel.ExMachina.PolymorphicEcto.params_for(__MODULE__, factory_name, attrs)
      end

      def string_params_for(factory_name, attrs \\ %{}) do
        Sozisel.ExMachina.PolymorphicEcto.string_params_for(__MODULE__, factory_name, attrs)
      end

      def params_with_assocs(factory_name, attrs \\ %{}) do
        Sozisel.ExMachina.PolymorphicEcto.params_with_assocs(__MODULE__, factory_name, attrs)
      end

      def string_params_with_assocs(factory_name, attrs \\ %{}) do
        Sozisel.ExMachina.PolymorphicEcto.string_params_with_assocs(
          __MODULE__,
          factory_name,
          attrs
        )
  ...
  defp verify_ecto_dep do
    unless Code.ensure_loaded?(Ecto) do
      raise "You tried to use Sozisel.ExMachina.PolymorphicEcto, but the Ecto module is not loaded. " <>
